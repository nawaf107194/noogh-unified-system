...
async def project_stats(token: str = Depends(verify_token)):
    ...
    for f in py_files:
        ...
        largest_files.append({
            "name": str(f.relative_to(SRC_ROOT)),
...
if __name__ == "__main__":
    print(f"🚀 Starting NOOGH Project API Server on port {PORT}...")
    uvicorn.run("project_api_server:app", host="0.0.0.0", port=PORT, reload=True)