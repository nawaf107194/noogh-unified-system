#!/usr/bin/env python3
"""
NOOGH Memory Distiller — مصفِّاة الذاكرة المركزية - FIXED v2
------------------------------------------------------
...
def _db_inject_belief(key: str, data: dict, utility: float = 0.90):
    """Inline replacement for unified_core.brain_tools.db_inject_belief."""
    try:
...
لديك مجموعة من المعتقدات المتناثرة حول موضوع '{topic}'.
بعضها قديم وبعضها متكرر.
...
{{
    ...
    def merge_redundant_beliefs(self):
        """يبحث عن المعتقدات من نفس الفئة ويدمجها"""
        ...
        for category, count in categories:
            ...
            items_query = """
                ...
                WHERE key LIKE ? ORDER BY updated_at ASC LIMIT 5
            """
            ...
            if merge_result and "merged_concept" in merge_result:
                ...
                self._execute_query(
                    f"DELETE FROM beliefs WHERE key IN ({placeholders})",
                    tuple(keys_to_delete)
                )
                # حقن الجديد — using inline function (no external deps)
                _db_inject_belief(new_key, {
