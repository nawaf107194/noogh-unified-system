...
@pytest.mark.parametrize("logits, index, expected", [
    (
        ...
        torch.tensor([0, 2]),
        torch.tensor([[1.7378, -inf, -inf], [-inf, logsumexp([4.0, 5.0, 6.0]), -inf]])
    ),
    (
        torch.tensor([[[1.0, 2.0]], [[3.0, 4.0]]]),
        torch.tensor([[0], [1]]),
        torch.tensor([[[2.1972]], [[-inf, logsumexp([3.0, 4.0])]]])
    ),
    (
        torch.tensor([[1.0], [2.0], [3.0], [4.0]]),
        torch.tensor([[0], [2], [3], [1]]),
        torch.tensor([[1.7378], [-inf, logsumexp([3.0])], [-inf, logsumexp([4.0])], [-inf, logsumexp([1.0, 2.0])]])
    ),
])
...
@pytest.mark.parametrize("logits, index", [
    ...
    (None, None),
    (torch.tensor([[1.0, 2.0], [3.0, 4.0]]), torch.tensor([2])),
])
...
@pytest.mark.parametrize("logits, index", [
    (torch.tensor([[1.0, 2.0], [3.0]]), torch.tensor([0, 3])),
    (torch.tensor([[1.0, 2.0], [3.0, 4.0]]), torch.tensor([-1, 2])),
])
